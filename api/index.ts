...
app.post('/api/campanha/parse-briefing', async (req, res) => {
  ...
  try {
    let responseText = ''
    if (type === 'url' && url) {
      const r = await googleAI.models.generateContent({ model: 'gemini-2.5-flash-preview-05-20', contents: [{ role: 'user', parts: [{ text: `${JSON_PROMPT}\n\nURL do briefing: ${url}` }] }] })
      ...
      const r = await googleAI.models.generateContent({ model: 'gemini-2.5-flash-preview-05-20', contents: [{ role: 'user', parts: [{ inlineData: { mimeType: mime, data: b64 } }, { text: JSON_PROMPT }] }] })
      responseText = r.candidates?.[0]?.content?.parts?.[0]?.text ?? '{}'
...
app.post('/api/campanha/generate-copies-full', async (req, res) => {
  ...
  try {
    const r = await googleAI.models.generateContent({ model: 'gemini-2.5-flash-preview-05-20', contents: [{ role: 'user', parts: [{ text: prompt }] }] })
